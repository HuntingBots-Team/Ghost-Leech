...
class SabnzbdStatus:
    ...
    def progress(self):
        return f"{self._info["percentage"]}%" # type: ignore

    def processed_raw(self):
        return (float(self._info["mb"]) - float(self._info["mbleft"])) * 1048576 # type: ignore

    def processed_bytes(self):
        return get_readable_file_size(self.processed_raw()) # type: ignore

    def speed_raw(self):
        try:
            return int(float(self._info["mb"]) * 1048576) / self.eta_raw() # type: ignore
        except:
            return 0
    ...
    def name(self):
        try:
            return self._info["filename"] # type: ignore
        except:
            return "Loading..."

    def size(self):
        return self._info["size"] # type: ignore

    def eta_raw(self):
        return time_to_seconds(self._info["timeleft"]) # type: ignore
    ...
    def status(self):
        ...
        state = self._info["status"] # type: ignore
